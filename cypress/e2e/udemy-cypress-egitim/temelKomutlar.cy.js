...
describe("Temel KOmutlar", () => {
  it("Temel cy.visit Kullanimi", () => {
    cy.visit("/"); // baseUrl tanimlandiysa
    cy.visit("/commands"); // baseUrl tanimlandiysa
    cy.visit("https://example.cypress.io/commands"); // baseUrl tanimlanmamissa
    cy.visit({
      url: "https://example.cypress.io/", // baseUrl tanimlanmamissa
      method: "GET",
    });
  });
  ...
  it("Temel Komutlar cy.log Kullanimi", () => {
    ...
    cy.log(" be.visible"); // boyle kullanma
    cy.log("Bir mesaj", ["bir mesaj", " iki mesaj", " uc mesaj"]); // Cikti:Bir mesaj,[ bir mesaj, iki mesaj, uc mesaj]
  });

  // only olunca sadece o kisim calisiyor=> it.only
  // skip olunca o kisim calismiyor diger kisimdan basliyor => it.skip
});
